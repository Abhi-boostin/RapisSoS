...
export function OfficerDashboard() {
  ...
  const handleStatusChange = async (dutyStatus: string) => {
    ...
    try {
      await setOfficerStatus(appState.phone, dutyStatus);
      setCurrentStatus(dutyStatus);
      setToast({ message: `Duty status updated to ${dutyStatus}`, type: 'success' });
    } catch (err) {
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
      const response = await acceptOfficerRequest(selectedRequestId, appState.phone);
  ...
  const handleDeclineRequest = async () => {
    ...
    setActionLoading(true);
    try {
      const response = await declineOfficerRequest(selectedRequestId, appState.phone);
      
      let message = 'Request declined';
      if (response.nextOfficer) {
        message += ` and reassigned to officer ${response.nextOfficer.badgeNumber}`;
      } else if (response.success === false) {
        message = response.message || 'No other officers available';
      }
