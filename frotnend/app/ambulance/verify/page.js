...
export default function VerifyAmbulance() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('http://localhost:4000/api/otp/verify-phone', {
        ...
        body: JSON.stringify({ phone, code, type: 'ambulance' })
      });
      ...
      // Check if ambulance needs to complete profile
      if (data.requiresProfile) {
        router.push(`/ambulance/details?phone=${encodeURIComponent(phone)}`);
      } else {
        // If profile exists, store the ambulance ID and token
