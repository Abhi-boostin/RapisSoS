...
export default function AmbulancePage() {
    ...
    useEffect(() => {
        const phone = localStorage.getItem('ambulancePhone');
        if (!phone) {
            router.replace('/ambulance/init');
        } else {
            setAmbulancePhone(phone);
            fetchRequests(phone);
    ...
    const fetchRequests = async (phone) => {
        try {
            const response = await fetch(`${endpoints.ambulanceRequests}?phone=${phone}`);
    ...
    const updateStatus = async (newStatus) => {
        try {
            ...
            const response = await fetch(endpoints.ambulanceStatus, {
                method: 'POST',
    ...
    const handleRequest = async (requestId, action) => {
        try {
            ...
            const endpoint = action === 'accept' 
                ? endpoints.ambulanceAcceptRequest(requestId)
                : endpoints.ambulanceDeclineRequest(requestId);

            const response = await fetch(endpoint, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ambulancePhone })
            });
                ...
                {requests.length > 0 ? (
                    <div className="space-y-4">
                        {requests.map((request) => (
                            <Card key={request.id} className="bg-red-50">
